  ...
  return (
    <div className="card-gradient rounded-xl border border-border p-6 flex flex-col items-center text-center gap-4 h-full">
      ...
      <div className="flex gap-3 mt-2">
        {member.website && (
          <SocialLink href={member.website} icon={<Globe className="h-5 w-5" />} label="Site pessoal" />
        )}
        {member.linkedin && (
          <SocialLink href={member.linkedin} icon={<Linkedin className="h-5 w-5" />} label="LinkedIn" />
        )}
        {member.instagram && (
          <SocialLink href={member.instagram} icon={<Instagram className="h-5 w-5" />} label="Instagram" />
        )}
      </div>
    </div>
  );
